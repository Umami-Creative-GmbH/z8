...
CREATE TABLE "time_entry_append_position" (
	...
	"admitted_tip_entry_id" uuid,
	"admitted_tip_hash" text,
	...
	CONSTRAINT "time_entry_append_position_admission_check" CHECK (("time_entry_append_position"."admission" = 'empty_history' AND "time_entry_append_position"."admitted_tip_entry_id" IS NULL AND "time_entry_append_position"."admitted_tip_hash" IS NULL AND "time_entry_append_position"."admitted_entry_count" = 0) OR ("time_entry_append_position"."admission" = 'verified_lineage' AND "time_entry_append_position"."admitted_tip_entry_id" IS NOT NULL AND "time_entry_append_position"."admitted_tip_hash" IS NOT NULL AND "time_entry_append_position"."admitted_entry_count" > 0)),
...
ALTER TABLE "time_entry_append_position" ADD CONSTRAINT "time_entry_append_position_admitted_tip_entry_id_time_entry_id_fk" FOREIGN KEY ("admitted_tip_entry_id") REFERENCES "public"."time_entry"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
