...
export const approvalPolicyCondition = pgTable(
	...
	(table) => [
		...
		foreignKey({
			...
			foreignColumns: [approvalPolicy.id, approvalPolicy.organizationId],
		}).onDelete("cascade"),
		foreignKey({
		...
		foreignKey({
			columns: [table.absenceCategoryId, table.organizationId],
			foreignColumns: [absenceCategory.id, absenceCategory.organizationId],
		}).onDelete("cascade"),
		foreignKey({
			columns: [table.employeeGroupId, table.organizationId],
			foreignColumns: [employeeGroup.id, employeeGroup.organizationId],
		}).onDelete("cascade"),
	],
);
...
export const approvalChainInstance = pgTable(
	...
	(table) => [
		...
		index("approvalChainInstance_org_entity_idx").on(
...
export const approvalChainStageInstance = pgTable(
	...
	(table) => [
		index("approvalChainStageInstance_org_chain_idx").on(
			table.organizationId,
			table.chainInstanceId,
		),
		uniqueIndex("approvalChainStageInstance_request_idx").on(table.approvalRequestId),
		uniqueIndex("approvalChainStageInstance_chain_order_idx").on(
			table.chainInstanceId,
			table.stepOrder,
		),
		foreignKey({
			columns: [table.chainInstanceId, table.organizationId],
			foreignColumns: [approvalChainInstance.id, approvalChainInstance.organizationId],
		}).onDelete("cascade"),
		foreignKey({
		...
		foreignKey({
			columns: [table.resolvedApproverEmployeeId, table.organizationId],
			foreignColumns: [employee.id, employee.organizationId],
		}),
		foreignKey({
