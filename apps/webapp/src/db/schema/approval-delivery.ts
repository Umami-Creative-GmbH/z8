...
// #291/#292: providers whose approval-card delivery has one durable owner.
export const APPROVAL_DELIVERY_PROVIDERS = ["telegram", "discord"] as const;
...
export const approvalDeliveryControl = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_control_provider_check", sql`${table.provider} IN ('telegram', 'discord')`),
	],
);
...
export const approvalDeliveryMessage = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_message_provider_check", sql`${table.provider} IN ('telegram', 'discord')`),
		check(
...
export const approvalDeliveryWork = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_work_provider_check", sql`${table.provider} IN ('telegram', 'discord')`),
		check(
