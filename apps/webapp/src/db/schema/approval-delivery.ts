...
// joined in #294 with review-only cards, Discord in #292.
export const APPROVAL_DELIVERY_PROVIDERS = ["telegram", "slack", "discord"] as const;
...
export const approvalDeliveryControl = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_control_provider_check", sql`${table.provider} IN ('telegram', 'slack', 'discord')`),
	],
);
...
export const approvalDeliveryMessage = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_message_provider_check", sql`${table.provider} IN ('telegram', 'slack', 'discord')`),
		check(
...
export const approvalDeliveryWork = pgTable(
	...
	(table) => [
		...
		check("approval_delivery_work_provider_check", sql`${table.provider} IN ('telegram', 'slack', 'discord')`),
		check(
