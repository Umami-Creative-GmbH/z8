...
export const COMPLETED_WORK_OPERATION_KINDS = ["close_active_work", "start_live_work", "create_completed_work"] as const;
export type CompletedWorkOperationKind = (typeof COMPLETED_WORK_OPERATION_KINDS)[number];

export const COMPLETED_WORK_WRITERS = ["web_clock_out", "direct_http", "runtime_demo"] as const;
...
export const completedWorkOperation = pgTable(
	...
	(table) => [
		...
		check("completed_work_operation_kind_check", sql`${table.kind} IN ('close_active_work', 'start_live_work', 'create_completed_work')`),
		check("completed_work_operation_writer_check", sql`${table.writer} IN ('web_clock_out', 'direct_http', 'runtime_demo')`),
		check(
