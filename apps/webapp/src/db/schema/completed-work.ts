...
export const completedWorkOperation = pgTable(
	...
	(table) => [
		...
		check(
			...
			sql`${table.kind} IN ('close_active_work', 'start_live_work', 'import_completed_work', 'import_open_work', 'create_completed_work', 'amend_completed_work', 'close_resume_work')`,
		),
		check(
