...
const REPORT = assessRollbackReadiness({
	...
	cards: {
		deliveryControls: [{ workflowType: "absence", provider: "telegram" }],
		presentationControls: [{ workflowType: "absence", provider: "telegram", mode: "review_only" }],
		openWork: [{ provider: "telegram", effect: "initial", count: 1 }],
		messages: { telegram: 2 },
		invocations: {},
...
describe("rollback readiness CLI", () => {
	...
	it("prints each section with its findings, the schema floor, then what the report cannot see", async () => {
		...
		expect(lines).toEqual([
			...
			"Approval cards: HOLD (delivery controls absence/telegram; actionable none; open work telegram initial 1)",
			...
			"    blocker transferred_approvals_pending (1)",
			...
			"Schema floor: 0099_work_balance_rebuild_intent (never narrow or drop below it; older code cannot read these rows)",
			"    0079_time_entry_append_position: append positions (3)",
			"    0083_completed_work_operation: receipt kind close_active_work (3)",
			"    0083_completed_work_operation: receipt writer web_clock_out (3)",
