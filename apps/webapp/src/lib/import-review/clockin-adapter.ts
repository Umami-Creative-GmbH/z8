...
function stagedWorkday(workday: ClockinWorkday & { id?: unknown }): {
	...
	const suspiciousFlags = workday.starts_at
		...
		: ["invalid_start"];
	const issues = [
		detectMissingMapping({ entityType: "work_period", providerSourceId, employeeId: null }),
	...
	return {
		row: {
			...
			matchTarget: { providerEmployeeId: workday.employee_id },
			rowStatus: issues.some((issue) => issue.issueType === "unmatched_employee") ? "needs_mapping" : "staged",
...
function stagedAbsence(absence: ClockinAbsence): {
	row: NormalizedImportRow;
	issues: ImportIssueDraft[];
} {
	...
	return {
		row: {
			...
			matchTarget: { providerEmployeeId: absence.employee_id },
			rowStatus: issues.some((issue) => issue.issueType === "unmatched_employee") ? "needs_mapping" : "staged",
			issueSeverity: issueSeverityFor(issues),
		},
		issues,
