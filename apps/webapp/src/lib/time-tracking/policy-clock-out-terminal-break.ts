...
export async function enforcePolicyClockOutTerminalBreakInTransaction(
	input: EnforcePolicyClockOutTerminalBreakInput,
	...
	const policyResult = await db.execute(sql`
		select
			...
			coalesce((
				select json_agg(json_build_object(
					'workingMinutesThreshold', rule.working_minutes_threshold,
					'requiredBreakMinutes', rule.required_break_minutes
				) order by rule.working_minutes_threshold)
				from work_policy_break_rule rule
				where rule.regulation_id = regulation.id
			), '[]'::json) as "breakRules"
	...
	if (policyRows[1]) {
		const fallback = object(policyRows[1]);
		if (
			...
			(fallback.assignmentPriority as number) >
				(policy.assignmentPriority as number) ||
			((fallback.assignmentPriority as number) ===
				(policy.assignmentPriority as number) &&
				(fallback.assignmentSpecificity as number) >=
					(policy.assignmentSpecificity as number))
		) {
			return fail();
		}
	}
	...
	const calculation = calculateBreakDeficit({
		...
		regulation: {
			id: policy.id,
			name: policy.name,
			maxUninterruptedMinutes: policy.maxUninterruptedMinutes as number | null,
			breakRules: policy.breakRules.map((value) => {
