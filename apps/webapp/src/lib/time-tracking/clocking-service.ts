...
type ClockingStore = {
	...
	getActivePeriod(employeeId: string, organizationId: string, workPeriodId?: string): Promise<ActivePeriod | null>;
	...
	closeActivePeriod(periodId: string, patch: Record<string, unknown>): Promise<{ id: string } | null>;
};
...
function entryValues(input: ClockingInput, type: "clock_in" | "clock_out", previousHash: string | null) {
...
export function createClockingService(deps: ClockingDependencies) {
	async function withinEmployeeTransaction<T>(input: ClockingInput, callback: (store: ClockingStore) => Promise<T>): Promise<T> {
	...
	return {
		clockIn: async (input: ClockInInput) =>
			withinEmployeeTransaction(input, async (store) => {
				...
				const entry = await store.insertEntry(
					entryValues(input, "clock_in", await store.getLatestHash(input.employeeId, input.organizationId)),
				);
		...
		clockOut: async (input: ClockOutInput) =>
			withinEmployeeTransaction(input, async (store) => {
				const activePeriod = await store.getActivePeriod(input.employeeId, input.organizationId, input.workPeriodId);
				if (!activePeriod) throw new ClockingConflictError("No active work period found");
				const timestamp = dateFromInstant(input.action.instant);
				const entry = await store.insertEntry(
					entryValues(input, "clock_out", await store.getLatestHash(input.employeeId, input.organizationId)),
				);
				const durationMinutes = Math.round((timestamp.getTime() - activePeriod.startTime.getTime()) / 60_000);
...
export const clockingService = createClockingService({
	...
	isOrganizationMember: async (employeeId, organizationId) => {
		const [member] = await tx.select({ id: employee.id }).from(employee).where(and(eq(employee.id, employeeId), eq(employee.organizationId, organizationId))).limit(1);
		return Boolean(member);
	},
	getActivePeriod: async (employeeId, organizationId, workPeriodId) => {
		const [period] = await tx.select({ id: workPeriod.id, startTime: workPeriod.startTime }).from(workPeriod).where(and(eq(workPeriod.employeeId, employeeId), eq(workPeriod.organizationId, organizationId), eq(workPeriod.isActive, true), isNull(workPeriod.endTime), ...(workPeriodId ? [eq(workPeriod.id, workPeriodId)] : []))).limit(1);
		return period ?? null;
	},
	getLatestHash: async (employeeId, organizationId) => {
		const [latest] = await tx.select({ hash: timeEntry.hash }).from(timeEntry).where(and(eq(timeEntry.employeeId, employeeId), eq(timeEntry.organizationId, organizationId))).orderBy(desc(timeEntry.createdAt)).limit(1);
		return latest?.hash ?? null;
	},
	insertEntry: async (values) => {
		const [entry] = await tx.insert(timeEntry).values(values as never).returning();
		if (!entry) throw new Error("Failed to create time entry");
		return entry;
	},
	insertActivePeriod: async (values) => {
		const [period] = await tx.insert(workPeriod).values(values as never).returning({ id: workPeriod.id });
		if (!period) throw new Error("Failed to create work period");
		return period;
	},
	closeActivePeriod: async (periodId, patch) => {
		const [period] = await tx.update(workPeriod).set(patch as never).where(and(eq(workPeriod.id, periodId), eq(workPeriod.isActive, true), isNull(workPeriod.endTime))).returning({ id: workPeriod.id });
		return period ?? null;
	},
	})),
});
