...
/**
 * Employee Report PDF Document Component
 */
const EmployeeReportPDF = ({ reportData }: { reportData: ReportData }) => (
	<Document>
...
export async function exportToPDF(reportData: ReportData): Promise<Uint8Array> {
	const blob = await pdf(<EmployeeReportPDF reportData={reportData} />).toBlob();
