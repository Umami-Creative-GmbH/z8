...
export async function getManagerDailyBriefingFromSources({
	...
	const approvals = buildSection<BriefingApprovalActionItem>(
		"approvals",
		sortActionItems(approvalItems) as BriefingApprovalActionItem[],
		approvalsResult,
	);
	const attendance = buildSection(
		"attendance",
		attendanceItems,
		firstRejected(shiftsResult, timeRecordsResult),
	);
	const sections = {
		approvals,
		attendance,
		absences: buildSection("absences", absenceItems, absencesResult),
		coverage: buildSection(
			"coverage",
			coverageItems,
			firstRejected(coverageRulesResult, shiftsResult),
		),
...
export async function getManagerDailyBriefing({
	currentEmployee,
	now = DateTime.now(),
...
const databaseSources: ManagerDailyBriefingSources = {
	...
	async getPublishedShifts({ organizationId, employeeIds, date }) {
		...
		return rows.flatMap((row): BriefingShift[] =>
			row.employeeId
				? [
						{
							id: row.id,
