...
describe("manager daily briefing logic", () => {
	it("detects missing and late clock-ins from published shifts only", () => {
		...
		expect(detectAttendanceExceptions({ now, shifts, records, graceMinutes: 5 })).toEqual([
			...
			expect.objectContaining({
				id: "attendance:shift-4",
				severity: "high",
				category: "attendance",
				title: "Katherine Johnson clocked in late",
			}),
		]);
	});
	...
	it("handles overnight shift attendance windows", () => {
		...
		expect(
			...
			expect.objectContaining({
				...
				severity: "critical",
				category: "attendance",
				title: "Grace Hopper has not clocked in",
			}),
		]);
	});
	...
	it("detects coverage risks for uncovered segments inside the rule window", () => {
		...
		expect(risks).toEqual([
			expect.objectContaining({
				...
				severity: "high",
				title: "Front desk is understaffed",
				description: "0 scheduled for 09:00-17:00; minimum is 1.",
			}),
		]);
	});
	...
	it("counts distinct employees for coverage staffing", () => {
		...
		expect(risks).toEqual([
			expect.objectContaining({
				...
				severity: "high",
				title: "Front desk is understaffed",
				description: "1 scheduled for 09:00-12:00; minimum is 2.",
			}),
		]);
	});
