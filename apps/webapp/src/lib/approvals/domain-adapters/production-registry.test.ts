...
describe("production approval domain adapter registry", () => {
	it("registers both migrated adapters and all five fail-closed domains", () => {
		const absence = {
		...
		for (const [workflowType, sourceType] of Object.entries(aliases)) {
	...
	it("fails every unmigrated operation before a database callback", async () => {
		...
		for (const workflowType of Object.keys(aliases) as Array<
