...
import { fromJSDate, parseISO, now, isValid } from './luxon-utils';
...
export const dateTimeSchema = z.union([
  ...
  z.string().datetime().transform(s => parseISO(s, 'utc')),
  z.custom<DateTime>(val => val && typeof val === 'object' && 'toJSDate' in val)
]).refine(
  val => isValid(val as DateTime),
...
export function formDateSchemaOptional() {
  return z.date().optional().nullable().transform(d => d ? fromJSDate(d, 'utc') : null);
}
...
export function apiDateSchemaOptional() {
  return z.string().datetime().optional().nullable().transform(s => s ? parseISO(s, 'utc') : null);
}
 ...
 */
export const birthdaySchemaOptional = z.date()
  .max(new Date(), 'Birthday must be in the past')
  .min(new Date(1900, 0, 1), 'Birthday must be after 1900')
  .optional()
  .nullable()
 ...
 */
export const absenceDateRangeSchema = z.object({
 ...
 */
export const workScheduleDateRangeSchema = z.object({
  effectiveFrom: z.date().transform(d => fromJSDate(d, 'utc')),
  effectiveUntil: z.date().optional().nullable().transform(d => d ? fromJSDate(d, 'utc') : null)
}).refine(
 ...
 */
export type InferDateTime<T extends z.ZodType<any, any, any>> = T extends z.ZodType<infer U, any, any>
  ? U extends DateTime ? U : never
  : never;
