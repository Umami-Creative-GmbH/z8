...
function expectBillingGuardBeforeWrite(source: string, commandName: string, writeMarker: string) {
	...
	expect(employeeValidationIndex, `${commandName} should validate employee organization membership`).toBeGreaterThanOrEqual(0);
	expect(guardIndex, `${commandName} should require billing before mutating`).toBeGreaterThanOrEqual(0);
	expect(allowedIndex, `${commandName} should check billing mutation access`).toBeGreaterThanOrEqual(0);
	expect(source).toContain("Billing is required to continue using time tracking");
	expect(writeIndex, `${commandName} should include expected write marker`).toBeGreaterThanOrEqual(0);
	expect(guardIndex, `${commandName} should guard after org validation`).toBeGreaterThan(employeeValidationIndex);
...
describe("bot time-tracking command billing guards", () => {
	it("guards clock-in before creating time entries", () => {
		expectBillingGuardBeforeWrite(readCommandSource("clock-in.ts"), "clock-in", ".insert(timeEntry)");
	});

	it("guards clock-out before creating time entries", () => {
		expectBillingGuardBeforeWrite(readCommandSource("clock-out.ts"), "clock-out", ".insert(timeEntry)");
	});
});
