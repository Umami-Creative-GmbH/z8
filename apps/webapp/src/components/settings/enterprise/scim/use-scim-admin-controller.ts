...
export function useScimAdminController(initialSetup: EnterpriseIdentitySetupResponse) {
	...
	const [lifecycle, setLifecycle] = useState<"creating" | "creation_failed" | "decommissioned" | null>(null);
	...
	const confirm = () => { if (!connectionId) return; const currentAction = destructive; const currentCredential = credentialId; setDestructive(null); setCredentialId(null); startTransition(async () => { try { if (currentAction === "revoke" && currentCredential) await revokeEnterpriseIdentityScimCredentialAction(connectionId, currentCredential); if (currentAction === "decommission") { await decommissionEnterpriseIdentityScimConnectionAction(connectionId); setLifecycle("decommissioned"); setStatus(null); return; } setStatus(await getEnterpriseIdentityScimStatusAction(connectionId)); setEvents(await listEnterpriseIdentityScimEventsAction(connectionId)); } catch { toast.error("Unable to update the SCIM connection."); } }); };
	return { connectionId, create, credential, destructive, events, eventsError, isPending, lifecycle, refresh, rotate, status, clearCredential: () => setCredential(null), requestRevoke: (id: string) => { setCredentialId(id); setDestructive("revoke"); }, requestDecommission: () => setDestructive("decommission"), cancelDestructive: () => { setCredentialId(null); setDestructive(null); }, confirm };
}
