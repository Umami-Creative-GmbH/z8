...
type ProfileFormApi = ReactFormExtendedApi<
	ProfileFormValues,
	FormValidateOrFn<ProfileFormValues> | undefined,
	FormValidateOrFn<ProfileFormValues> | undefined,
	FormAsyncValidateOrFn<ProfileFormValues> | undefined,
	FormValidateOrFn<ProfileFormValues> | undefined,
	FormAsyncValidateOrFn<ProfileFormValues> | undefined,
	FormValidateOrFn<ProfileFormValues> | undefined,
	FormAsyncValidateOrFn<ProfileFormValues> | undefined,
...
export function ProfileForm({ user }: ProfileFormProps) {
	...
	const [today] = useState(() => new Date(Temporal.Now.instant().toString()));
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			try {
				...
				toast.error(
					profileResult.error ||
						t("settings.profile.updateFailed", "Failed to update profile"),
				);
			} catch {
				toast.error(
					t("settings.profile.updateFailed", "Failed to update profile"),
	...
	const selectedBirthday = useStore(
		form.store,
		(state) => state.values.birthday,
	);
	...
	const avatarUpdateMutation = useMutation({
		...
		onSuccess: (result) => {
			if (result.success) {
				toast.success(
					t("settings.profile.avatarUploaded", "Avatar uploaded successfully"),
				);
				void Promise.all([
					queryClient.invalidateQueries({
						queryKey: queryKeys.profile.current(),
				...
				toast.error(
					result.error ||
						t("settings.profile.avatarSaveFailed", "Failed to save avatar"),
				);
			}
		},
		onError: () => {
	...
	const {
		...
		onError: (error) => {
			toast.error(
				error?.message ||
					t("settings.profile.avatarUploadFailed", "Failed to upload avatar"),
			);
		},
	});
	...
	useEffect(() => {
		...
		async function loadEmployeeData() {
			...
			form.setFieldValue(
				"gender",
				(emp?.gender as ProfileFormValues["gender"] | null) || "",
			);
			form.setFieldValue("pronouns", emp?.pronouns || "");
			form.setFieldValue(
				"birthday",
				birthdayValueToCalendarDate(emp?.birthday),
			);
	...
	const removeAvatarMutation = useMutation({
		...
		onSuccess: (result) => {
			if (result.success) {
				toast.success(
					t("settings.profile.avatarRemoved", "Avatar removed successfully"),
				);
				queryClient.invalidateQueries({
					queryKey: queryKeys.profile.current(),
				});
				...
				toast.error(
					result.error ||
						t("settings.profile.avatarRemoveFailed", "Failed to remove avatar"),
				);
			}
		},
	...
	return (
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					<CardTitle>
						{t("settings.profile.information", "Profile Information")}
					</CardTitle>
					<CardDescription>
						{t(
...
function ProfileUploadProgress({
	progress,
	t,
}: {
	progress: number;
	...
	return (
		<div className="space-y-1">
			...
			<p className="text-xs text-muted-foreground">
				{t("settings.profile.uploading", "Uploading")} {progress}%
			</p>
		</div>
	);
}

function ProfileEmail({ email, t }: { email: string; t: TranslationFunction }) {
	return (
		<div className="space-y-2">
...
function ProfilePersonalInformation({
	...
	selectedGender: ProfileFormValues["gender"];
	t: TranslationFunction;
	today: Date;
}) {
	return (
		<div className="border-t pt-6 space-y-6">
			...
			<ProfileGenderField form={form} selectedGender={selectedGender} t={t} />
			<ProfilePronounsField form={form} isSubmitting={isSubmitting} t={t} />
...
interface ProfileAvatarAndNamesProps {
	avatarImage: string;
	...
	presence: ReturnType<typeof useEmployeeClockStatuses>;
...
interface ProfileEditorProps extends ProfileAvatarAndNamesProps {
	selectedBirthday: Date | null;
	selectedGender: ProfileFormValues["gender"];
	today: Date;
...
function ProfileEditor(props: ProfileEditorProps) {
	...
	return (
		<form action={() => form.handleSubmit()} className="space-y-6">
			...
			<ProfileEmail email={user.email} t={t} />
...
function ProfileAvatarAndNames({
	avatarImage,
	currentEmployeeId,
	displayName,
	form,
	...
	return (
		<div className="flex flex-col gap-6 sm:flex-row sm:items-start">
			<input
				...
				className="hidden"
				aria-label={t(
					"settings.profile.uploadProfilePicture",
			...
			<div className="relative mx-auto shrink-0 sm:mx-0 sm:mt-7">
				<UserAvatar
					...
					clockStatus={
						currentEmployeeId
							? presence.getStatus(currentEmployeeId)
							: "unknown"
					}
				/>
				{isUploadingAvatar && (
				...
				{avatarImage && (
					<button
						...
						disabled={isUploadingAvatar || isSubmitting || removeAvatarPending}
...
function ProfileGenderField({
	...
	const options = [
		{
			value: "male",
			label: t("settings.profile.gender.male", "Male"),
			icon: IconGenderMale,
		},
		{
			value: "female",
			label: t("settings.profile.gender.female", "Female"),
			icon: IconGenderFemale,
		},
		{
			value: "other",
			label: t("settings.profile.gender.other", "Other"),
	...
	return (
		<div className="space-y-2">
			<Label id="gender-label">
				{t("settings.profile.gender.label", "Gender")}
			</Label>
			<div
				role="radiogroup"
			...
			>
				{options.map((option) => {
					...
					return (
						// biome-ignore lint/a11y/useSemanticElements: Preserve the existing styled radio-button interaction.
						<button
							key={option.value}
							type="button"
							role="radio"
							aria-checked={selectedGender === option.value}
							onClick={() => form.setFieldValue("gender", option.value)}
							className={cn(
	...
	return (
		...
		>
			{(field) => {
				...
				const customLabel = t(
					"settings.profile.pronouns.custom",
					"Custom pronouns",
				);
					...
					<TFormItem>
						...
						<Select
							...
							onValueChange={(nextValue) =>
								field.handleChange(
									nextValue === CUSTOM_PRONOUN_VALUE ? "" : nextValue,
								)
							}
						>
							<TFormControl hasError={hasError}>
							...
							<SelectContent>
								...
								<SelectItem value={CUSTOM_PRONOUN_VALUE}>
									{customLabel}
								</SelectItem>
							</SelectContent>
						</Select>
...
function ProfileImprovementPreference({
	form,
	isSubmitting,
	t,
}: {
