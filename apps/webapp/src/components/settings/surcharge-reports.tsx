...
export function SurchargeReports({ organizationId }: SurchargeReportsProps) {
	...
	const loadCalculationsRef = useRef<(filters: FilterValues) => Promise<void>>(
		async () => {},
	);

	loadCalculationsRef.current = async (filters: FilterValues) => {
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			await loadCalculationsRef.current(value);
		},
	});
	...
	useEffect(() => {
		...
		void loadCalculationsRef.current(appliedFilters.current);
	}, [organizationId]);
	...
	return (
		<div className="space-y-4">
			...
			<div className="grid gap-4 md:grid-cols-4">
				...
				<SummaryCard
					label={t(
						"settings.surcharges.reports.summary.baseHours",
						"Base hours",
					)}
			...
			<Card>
				<CardContent>
					{isLoading && displayRows.length === 0 ? (
						...
						>
							{t(
								"settings.surcharges.reports.loading",
								...
								<TableRow>
									<TableHead className="w-10">
										<span className="sr-only">
											{t(
												"settings.surcharges.reports.table.details",
												"Details",
											)}
										</span>
									</TableHead>
									...
									<TableHead>
										{t(
											"settings.surcharges.reports.table.employee",
											"Employee",
										)}
									</TableHead>
									<TableHead>
									...
									<TableHead>
										{t(
											"settings.surcharges.reports.table.qualifying",
											"Qualifying",
										)}
									</TableHead>
									<TableHead>
									...
									<TableHead>
										{t(
											"settings.surcharges.reports.table.percentage",
											"Percentage",
										)}
									</TableHead>
									<TableHead>
											...
											<TableRow className="tabular-nums">
												<TableCell>
													<Button
														...
														onClick={() =>
															setExpandedId(isExpanded ? null : calculation.id)
														}
													>
														{isExpanded ? (
