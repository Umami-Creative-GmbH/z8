...
function useApiKeyPageViewModel({
	organizationId,
	initialApiKeys,
	...
	const [deleteDialogKey, setDeleteDialogKey] = useState<ApiKeyResponse | null>(
		null,
	);
...
type ApiKeyPageViewModel = ReturnType<typeof useApiKeyPageViewModel>;

function ApiKeysCard({ viewModel }: { viewModel: ApiKeyPageViewModel }) {
	const {
		apiKeys,
		copiedKeyId,
		handleCopyPrefix,
					...
					<Table>
						<TableHeader>
							<TableRow>
								...
								<TableHead>
									{t("settings.apiKeys.prefix", "Key Prefix")}
								</TableHead>
								<TableHead>
												...
												<Tooltip>
													<TooltipTrigger asChild>
														<Button
															...
															onClick={() =>
																handleCopyPrefix(apiKey.prefix || "", apiKey.id)
															}
														>
...
function ApiKeyDialogs({ viewModel }: { viewModel: ApiKeyPageViewModel }) {
	const {
		createDialogOpen,
		deleteDialogKey,
		deleteMutation,
			...
			>
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							onClick={() =>
								deleteDialogKey && deleteMutation.mutate(deleteDialogKey.id)
							}
...
function ApiKeyPageView({
	viewModel,
}: {
	viewModel: ReturnType<typeof useApiKeyPageViewModel>;
}) {
	const { canCreateMore, keyCount, setCreateDialogOpen, t } = viewModel;
		...
		<div className="p-6">
			<div className="mx-auto max-w-4xl">
				...
				<div className="space-y-6">
					<ApiKeysCard viewModel={viewModel} />
			...
			<ApiKeyDialogs viewModel={viewModel} />
		</div>
	);
}

export function ApiKeyPageClient(props: ApiKeyPageClientProps) {
	const viewModel = useApiKeyPageViewModel(props);
	return <ApiKeyPageView viewModel={viewModel} />;
}
