	...
	const effect = tracer.startActiveSpan(
		...
		(span) => {
			return Effect.gen(function* (_) {
				...
				// Verify owner role (only owners can update fiscal year settings)
				if (memberRecord.role !== "owner") {
					yield* _(
						Effect.fail(
							new AuthorizationError({
				...
				// Update the organization fiscal year start month
				yield* _(
					Effect.tryPromise({
						try: async () => {
							await db
							...
							return new ValidationError({
								message:
									error instanceof Error
										? error.message
										: "Failed to update organization fiscal year start month",
								field: "month",
							});
						},
					}),
