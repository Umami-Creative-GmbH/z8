...
export function EmployeeDetailPageClient({
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			if (!result) {
				toast.error(
					t("settings.employees.details.toast.unexpectedError", "An unexpected error occurred"),
				);
				return;
			}
			...
			if (result.success) {
				toast.success(
					t("settings.employees.details.toast.updateSuccess", "Employee updated successfully"),
				);
				push("/settings/employees");
			} else {
				toast.error(
					result.error ||
						t("settings.employees.details.toast.updateError", "Failed to update employee"),
				);
			}
		},
	});
	...
	if (!hasEmployee && !isLoading) {
		return (
			<div className="flex flex-1 items-center justify-center p-6">
				<NoEmployeeError
					feature={t("settings.employees.details.noEmployeeFeature", "manage employees")}
				/>
			</div>
		);
	}
		...
		return (
			<div className="flex flex-1 flex-col gap-4 p-4">
				<div
					...
					role="status"
					aria-label={t("settings.employees.details.loadingAriaLabel", "Loading employee data")}
				>
