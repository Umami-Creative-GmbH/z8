...
function DiagnosticsItemRow({
	...
	return (
		<div className="flex flex-col gap-3 border-b py-4 last:border-b-0 sm:flex-row sm:items-start sm:justify-between">
			...
			<div className="wrap-break-words font-mono text-sm text-muted-foreground sm:text-right">
				{item.value}
			</div>
		</div>
	);
...
type DiagnosticsClientAction =
	...
	| {
			type: "smtpOverrideChanged";
			update: (current: SmtpOverrideState) => SmtpOverrideState;
	  }
	| { type: "emailStarted" }
	| {
			type: "emailSucceeded";
			result: { recipient: string; messageId?: string };
	  }
				...
				<div className="space-y-2">
					...
					<output className="sr-only" aria-live="polite">
						{t(
							...
							{
								status: statusLabels[snapshot.overallStatus],
								date: snapshot.fetchedAt,
							},
						)}
					</output>
