	...
	headers: z
		.array(
			z.object({
				key: z
					.string()
					.max(100, { message: "Header key cannot exceed 100 characters" }),
				label: z
					.string()
					.max(200, { message: "Header label cannot exceed 200 characters" }),
			}),
		)
...
function sanitizeForExcel(
	...
	return data.map((row) => {
		...
		for (const [key, value] of Object.entries(row)) {
			if (
				typeof value === "string" &&
				formulaChars.some((char) => value.startsWith(char))
			) {
