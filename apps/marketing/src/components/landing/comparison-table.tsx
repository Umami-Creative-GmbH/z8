...
type ComparisonTableProps = {
	copy: LandingCopy["comparisons"];
};

export function ComparisonTable({ copy }: ComparisonTableProps) {
	return (
		<section className="relative z-10 px-8 py-28 lg:px-16">
			<div className="mx-auto max-w-3xl">
				...
				>
					...
					>
						<span style={{ color: v("textTertiary") }}>{copy.featureColumn}</span>
						<span className="text-center" style={{ color: v("text") }}>
							{copy.z8Column}
						</span>
						<span className="text-center" style={{ color: v("textQuaternary") }}>
							{copy.othersColumn}
						</span>
					</div>
					{copy.items.map((c, i) => (
						<div
							...
							style={{
								borderBottom: i < copy.items.length - 1 ? `1px solid ${v("border")}` : "none",
