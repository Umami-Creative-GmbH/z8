...
export function Footer({ copy }: FooterProps) {
	...
	return (
		<footer
			className="relative z-10 px-8 pb-8 pt-20 lg:px-16"
			style={{ borderTop: `1px solid ${t.border}`, transition: "border-color 0.4s ease" }}
		>
			<div className="mx-auto max-w-6xl">
				<div className="mb-16 grid gap-12 md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr]">
					<div>
						<span className="mb-4 block text-[24px] font-black tracking-[-0.02em]">
							{copy.brand}
						</span>
						<p
							className="mb-6 max-w-[220px] text-[14px] leading-relaxed"
					...
					{Object.entries(copy.linkGroups).map(([category, links]) => (
						<div key={category}>
							<h4
								className="mb-4 text-[12px] font-bold uppercase tracking-[0.12em]"
