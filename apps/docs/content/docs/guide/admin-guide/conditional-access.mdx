...
Role Templates provide a way to bundle permissions and conditional access policies into reusable configurations. When users join your organization via SSO or SCIM, they can be automatically assigned the appropriate role template based on their identity provider groups.
...
5. Optionally link an access policy
