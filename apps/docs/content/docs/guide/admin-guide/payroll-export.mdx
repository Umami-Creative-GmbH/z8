...
Workday support currently focuses on secure credential storage, tenant validation, and integration setup.
...
4. Choose the employee matching strategy.
5. Use **Test Connection** to validate the tenant and credentials.

Workday credentials are stored securely as organization-scoped secrets in Z8.

## Workday Status

If your organization is using the current v1 Workday path, verify whether the export run validates connectivity only or posts records before relying on it operationally.

Today the safest assumption is that Workday setup and validation are available, while end-to-end posting maturity may still depend on your rollout path.

<Callout type="warn" title="Validate before production use">
  Do not assume Workday has the same export maturity as DATEV, Lexware, Sage, Personio, or SAP SuccessFactors. Run a controlled test with your payroll and Workday teams first.
</Callout>
...
- Treat Workday as a setup-and-validation path until your team confirms live posting behavior.
