...
SCIM provisioning is organization-scoped. It automates member lifecycle changes for one organization at a time and does not create cross-organization access on its own.

Each organization gets its own SCIM endpoint configuration and bearer token.
...
```
SCIM Endpoint: https://your-z8-domain.com/api/scim/v2
...
<Callout type="info" title="Organization scope">
  A SCIM token only applies to the organization that generated it. Configure SCIM separately for each organization you manage.
</Callout>
...
4. If disabled, the user stays inactive until your organization reviews the new member and related employee setup in the [Employee Management](/docs/guide/admin-guide/employee-management) flow
