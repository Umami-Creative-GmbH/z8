...
For managed PostgreSQL providers that require TLS, keep the individual connection variables and set the SSL mode explicitly. If the provider documents `sslmode=verify-full sslrootcert=/path/to/provider-ca.pem`, configure:
...
Use `POSTGRES_SSL_CA_CERT` instead of `POSTGRES_SSL_ROOT_CERT_PATH` only when storing the PEM certificate content as an environment variable is preferable to mounting a CA file.
