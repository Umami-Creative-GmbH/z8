...
For SCIM specifically, keep expectations organization-scoped: organization owners and admins configure it through **Enterprise Identity Setup** at `/settings/enterprise/identity-setup` after SSO is configured.
...
SCIM support is part of the enterprise auth stack for provisioning through **Enterprise Identity Setup**. It should be understood as an organization-scoped integration, not a cross-tenant identity plane.
...
- SCIM setup provides the base URL and bearer token in the wizard, with the token shown only once
- provisioning verification depends on the identity provider pushing a test user or group change back to that organization's SCIM endpoint
...
| Auth Method | Signal | App Type |
|-------------|--------|----------|
| Cookie | Any | Webapp |
| Bearer token | `X-Z8-App-Type: mobile` | Mobile |
| Bearer token | `X-Z8-App-Type: desktop` | Desktop |
| Bearer token | No explicit app header | Legacy fallback based on user agent |
