...
Runtime database connections use individual environment variables rather than a single connection string:
...
Managed PostgreSQL providers may require TLS. For providers that document `sslmode=verify-full sslrootcert=/path/to/provider-ca.pem`, use:
...
`POSTGRES_SSL_CA_CERT` can be used instead when the CA certificate PEM content is supplied through the secret store rather than mounted as a file.
