...
- SCIM is part of the enterprise auth stack and is configured through **Enterprise Identity Setup** at `/settings/enterprise/identity-setup` after org-scoped SSO is configured.
...
Desktop and mobile clients use a browser-based login handoff with a one-time authorization code:

```text
/api/auth/app-login?app=mobile&redirect=z8mobile://auth/callback&challenge=<code-challenge>
/api/auth/app-login?app=desktop&redirect=z8://auth/callback&challenge=<code-challenge>
```
...
3. After sign-in, the route redirects back to the native deep link with a one-time code.
4. The client exchanges the code and its verifier through `/api/auth/app-exchange` for a bearer session token.

---

## Native Client Requests

Native clients authenticate API requests with bearer session tokens. The app exchange endpoint uses `X-Z8-App-Type` to identify the client for its code exchange.

<Callout type="info" title="Mobile-Only Routes">
  Mobile API routes under `/api/mobile/*` require a bearer token plus `X-Z8-App-Type: mobile`.
</Callout>
