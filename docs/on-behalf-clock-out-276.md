...
drizzle/0092_on_behalf_clock_out_writer.sql                 # adds the manager_on_behalf writer
```
...
PostgreSQL 16 database with the fresh migration chain through `0092`. The target's
...
through `0092`, the suite (22 tests), its adapter tests and the #275 suite passed
**59/59**.
